...
class MixtureOfGaussian:
    ...
    def EM_Diag(self, train_1d, vowel):
        ...
        # prior = [1. / self.num_gaussian] * self.num_gaussian
        ...
        for it in range(self.num_iter):
            # compute log likelihood for stopping iteration
            temp_prob = 0
            ...
            for i in range(self.num_gaussian):
                means[i] = sum([cond_prob_count[j][i] * float(train_1d[j]) for j in range(num_data)]) / float(prior_count[i])
    ...
    def TestDiagMoG(self, test_data, test_labels):
        ...
        # print predict_vowel, test_labels[i]
        f1.close()
